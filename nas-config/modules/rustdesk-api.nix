...
#    already configured), then log in via username/password (the account
#    and password were auto-created when you first logged into the admin
#    console via OIDC in step 6).
# 8. (Optional: if you want OIDC-only login to the web admin console and
#    don't need desktop clients to connect via API Server, set
#    services.rustdesk-api.disablePwdLogin = true and redeploy. This
#    breaks desktop-client login — leave it false if you want clients to
#    work. With it false, Pocket ID still gates *account creation* via
#    AutoRegister; password is just how desktop clients authenticate once
